...
def predict_folder_pixel_abs(input_folder, target_folder):
    ...
    ckp_path = '/workspace/brain_best.pth'
    model_state_dict = torch.load(ckp_path, map_location='cpu')
    
    # model_state_dict = torch.load('/home/zhaoxiang/mood_challenge/docker_example/scripts/brain_best.pth', map_location='cpu')
    ...
    for f in os.listdir(input_folder):
        ...
        with torch.no_grad():
            ...
            for i in range(img_tensor.shape[0]):
                ...
                # torchvision.utils.save_image(input ,'input_brain.png')
                # torchvision.utils.save_image(output ,'output_brain.png')
        ...
        nib.save(final_nimg, target_file)
        nib.save(final_nimg, 'output.nii.gz')
